...
export class NgChat implements OnInit, IChatController {
    ...
    public receiveEmogi(window, $event){
        console.log($event.emoji);
        let windowIndex = this.windows.indexOf(window);
        if (windowIndex >= 0)
        {
            let messageInputToFocus = this.chatWindowInputs.toArray()[windowIndex];
            messageInputToFocus.nativeElement.value 
                = messageInputToFocus.nativeElement.value + " " +$event.emoji.native;
        }
    }
    ...
    */
    onChatInputTyped(event: any, window: Window): void
    {
        ...
        {
            case 13:
                if (window.newMessage && window.newMessage != "")
                {
    ...
    /*Send File function*/
    onLoadSendFile($event, window: Window){
        ...
        this.serviceUpload.uploadFile($event);

        /*fileInfo.name = $event.target.files[0].name;
        fileInfo.lastModified = $event.target.files[0].lastModified;
        fileInfo.size = $event.target.files[0].size;
        fileInfo.type = $event.target.files[0].type;
        fileInfo.webkitRelativePath = $event.target.files[0].webkitRelativePath;*/
        message.file = fileInfo; 
        
        window.messages.push(message);
        
        this.adapter.sendFile(message);
    }
}
